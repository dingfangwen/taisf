 ...
 * @since 1.0
 */
@Service(PushHandleConstant.HANDLE_APPLE_USER)
public class ApplePushUserHandle extends PushAppleConstant implements PushHandle {
    ...
    private void instanceAppleService(){
        if (Check.NuNStr(CERTIFICATE_PATH_USER)){
            throw new ApplePushException("苹果证书路径为空");
        }

        if (Check.NuNStr(CERTIFICATE_PASSWORD_USER)){
            throw new ApplePushException("苹果证书密码为空");
        }

        ApnsService service = null ;
        if(APPLE_ENV.equals("dev")){
            service = APNS.newService().withCert(CERTIFICATE_PATH_USER, CERTIFICATE_PASSWORD_USER)
